...
class Parser {
     ...
     */
    lex(code) {
        return new lexer_1.Lexer(code).getTokens();
    }
    /**
    ...
    renderBlock(tokens) {
        ...
        // on each call
        const placeholder = (str) => `\$\{${count++}:${str}\}`;
     ...
     */
    getParamTag(c, type, t, name, p, desc) {
     ...
     *                                   parameters
     */
    renderParamTags(tokens, blockList, placeholder) {
        // Get column spacing from configuration object
        const column = this.config.get('columnSpacing');
        ...
        if (tokens.params.length && tokens.type !== 'variable') {
            ...
            for (const param of tokens.params) {
                ...
                blockList.push(this.getParamTag(cSpace, type, tSpace, name, pSpace, desc));
            }
        }
        return blockList;
    }
     ...
     */
    getReturnTag(type, spacing, desc) {
     ...
     *                                   return tag
     */
    renderReturnTag(tokens, blockList, placeholder) {
        // Get column spacing from configuration object
        const column = this.config.get('columnSpacing');
        // Determine whether or not to display the return type by default
        const defaultReturnTag = this.config.get('defaultReturnTag');
     ...
     */
    renderVarTag(tokens, blockList, placeholder) {
        // Add special case of variable blocks
        if (tokens.type === 'variable') {
            // Empty line
            blockList.push('');
            // Format type to be tab-able
            const type = placeholder(tokens.varType ? tokens.varType : `[type]`);
            // Var type
     ...
     */
    tokenize(code, next = '', tokens = null) {
