...
const character_parser_1 = require("character-parser");
...
class Lexer {
    ...
    attrs() {
        ...
        if ('(' === this.input.charAt(0)) {
            ...
            let state = character_parser_1.defaultState();
                    ...
                    if (whitespaceRe.test(attrStr[j])) {
                        for (let x = j; x < attrStr.length; x++) {
                            if (!whitespaceRe.test(attrStr[x])) {
                                ...
                                // of the value
                                return !character_parser_1.isPunctuator(attrStr[x]) ||
                    ...
                    expected \` \`, \`\\n\`, \`\t\`, \`,\`, \`!\` or \`=\``);
                        ...
                        case 'key':
                            ...
                            if (key === '' && quoteRe.test(attrStr[i])) {
                                ...
                                state = character_parser_1.defaultState();
                            }
                            else {
                        ...
                        case 'value':
                            ...
                            state = character_parser_1.parseChar(attrStr[i], state);
    ...
    bracketExpression(skip) {
        ...
        try {
            ...
            range = character_parser_1.parseUntil(this.input, end, { start: skip + 1 });
        }
        catch (ex) {
            throw ex.message;
        }
     ...
     */
    checkNesting(exp) {
        if (character_parser_1.parse(exp).isNesting()) {
