...
class PlSqlNavigator {
    ...
    static async searchExternal(search, search_cb, parseFn) {
        const globCmd = this.getGlobCmd(search, search_cb);
        for (let searchFld of globCmd.searchFld) {
            globCmd.params.cwd = searchFld;
            const resultSearch = await this.searchExternalGlob(globCmd, Object.assign({}, search), parseFn);
            if (resultSearch)
                return resultSearch;
    ...
    static searchExternalGlob(globCmd, search, parseFn) {
        return new Promise((resolve, reject) => {
            let files;
    ...
    static readFile(file) {
        return new Promise((resolve, reject) => {
            fs.readFile(file, (err, data) => {
                ...
                return resolve(plsqlParser_1.default.parseFile(file, dataS));
            });
        });
    }
}
PlSqlNavigator.useEncoding = 'utf8';
