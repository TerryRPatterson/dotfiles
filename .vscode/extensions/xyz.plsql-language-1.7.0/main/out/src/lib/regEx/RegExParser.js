    ...
    static getSymbols(text, fileName) {
        ...
        while (found = this.regExp.exec(text)) {
            if (found[2]) {
                ...
                if ([14 /* view */, 13 /* trigger */, 15 /* table */]
...
RegExpParser.regSymbolsCreate = `(?:(create)(?:\\s+or\\s+replace)?\\s*(?:(?:no\\s+)?force|global\\s+temporary)?\\s*)`;
RegExpParser.regSymbols = `(?:\\b(function|procedure|package|trigger|view|table)\\b(?:\\s+(body))?)\\s+`;
...
RegExpParser.regSpecCondition = `(?:\\$\\b(?:if|elsif)\\b.*\\s*\\$\\bthen\\b|\\$\\b(?:end|else)\\b)`;
